...
Request body (JSON)
-------------------
Expected JSON shape (field names follow the current code):
...
```json
{
  ...
  "vehicle": { "color": "blue", "plate": "XYZ987", "capacity": 4, "vehicleType": "car" }
}
```
...
- The code property for the vehicle is `vehicle`. If you rename it again, update routes, services, models and this README consistently.
...
----------------------------------------------------------------
Below is a JSON example in JSONC-style (JSON with comments). The comments explain requirements and constraints — your client should send plain JSON (without comments). The keys match the current code (`vehicle`).

```jsonc
{
  "fullname": {
    "firstname": "Sam",          // required, string, min length 3
    "lastname": "Driver"         // optional, string, min length 3 when present
  },
  "email": "sam.driver@example.com", // required, valid email, unique
  "password": "driverPass123",      // required, string, min length 6 (hash before store)
  "vehicle": {
    "color": "blue",                 // required, string, min length 3
    "plate": "XYZ987",               // required, string, min length 3
...
Note: JSONC is shown only for documentation. Remove the comments when sending the actual request body.

Success response (201 Created) — JSON example with comments
---------------------------------------------------------
```jsonc
{
  "message": "Captain created successfully",
  "captainId": "<captain id>",    // string ID returned by MongoDB (e.g. "642a...")
  "token": "<jwt token>"          // optional: JWT token generated by captain.generateAuthToken()
}
```

Validation error response (400 Bad Request) — JSON example
-------------------------------------------------------
```json
{
  "errors": [
...
Conflict response (409) when email already exists
--------------------------------------------------
```json
{ "message": "Email already registered" }
```
...
------------------------------
- The code property for the vehicle is `vehicle`. If you rename it again, update routes, services, models and this README consistently.
- Ensure passwords are hashed before saving (see `captain.model.js` static method `hashPassword`).
- Validate unique email at both service and DB/model layers to avoid race conditions.
- The `captain.controller.js` uses `captain.generateAuthToken()` to create a token; include it in the response if your flow requires immediate authentication.
